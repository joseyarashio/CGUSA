...
var WaveSurfer = {
	...
	zoomin: function () {
		if(wavesurfer.params.minPxPerSec<100)
		{
	...
	zoomout: function () {
		if(wavesurfer.params.minPxPerSec>=20)
		{
			wavesurfer.params.minPxPerSec /= 2;
			wavesurfer.bindMarks();
			wavesurfer.bindDragNDrop();				
	...
	mark2: function (time,options) {//mark時間根據所算出來的lrc
        var my = this;
        var timings = this.timings2(0);
	...
	timings2: function (time) {//傳遞起始時間與總長度
		this.markStartPosition=time;
        var position = this.markStartPosition || 0;
        var duration = this.backend.getDuration() || 1;
